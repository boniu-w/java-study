...
```tex
在Java中，List<T> 是一个使用泛型的接口，用于表示一个有序的集合，其中可以包含重复的元素。泛型 <T> 表示列表中元素的类型，它可以是任何非原始类型，包括类、接口甚至是其他泛型类型。下面是使用 List 泛型的一些常见情况及其解释：

基本类型（使用包装类）:
List<Integer>：一个只能包含 Integer 类型对象的列表。
List<Double>：一个只能包含 Double 类型对象的列表。
List<Character>：一个只能包含 Character 类型对象的列表。


自定义类型:
List<Person>：假设有一个自定义的 Person 类，这个列表就只能包含 Person 对象。
List<Book>：如果有 Book 类，这个列表就是存储 Book 对象的。

泛型的泛型（嵌套泛型）:
List<List<String>>：一个列表，其元素是字符串列表。
List<Map<String, Integer>>：一个列表，其元素是字符串到整数的映射。

通配符的使用:
List<?>：一个可以接受任何类型列表的引用，但不能添加任何元素（除了 null）。
List<? extends Number>：可以指向任何继承自 Number 的类型的列表，如 Integer、Double 等，但只能读取元素，不能添加元素（除非是 null 或者 Number 的子类实例）。
List<? super Number>：可以指向任何可以是 Number 类或其超类类型的列表，如 Number、Object，可以添加 Number 或其子类的实例，但读取时只知道是 Number 或其超类类型。

边界约束:
List<T extends Comparable<T>>：列表中的元素必须实现 Comparable 接口，通常用于可以排序的元素。
List<T extends MyClass & MyInterface>：元素必须是 MyClass 的子类并且实现了 MyInterface。

无界泛型:
List<?>：已经提及，表示可以是任何类型的列表，但在使用时受到限制。
了解这些不同的泛型使用场景有助于编写更加灵活、类型安全的Java代码。在实际编程中，选择合适的泛型约束可以提高代码的清晰度和健壮性。
```
...
为了避免以上情况，我们手动定义一个静态常量来人为定义

serialVersionUID

，因为一旦手动定义了，虚拟机就不会再进行计算了，这也就是你上面写的那就话。

# 140. 运算符
```text
Java中的运算符优先级决定了在没有明确使用括号改变计算顺序的情况下，运算符被评估的顺序。优先级高的运算符会先于优先级低的运算符进行计算。以下是一份常见的Java运算符优先级列表，从高到低排列：

后缀运算符：.（成员访问）、[]（数组访问）、()（方法调用）、++（自增）、--（自减）
类型转换和强制类型转换
乘性运算符：*（乘）、/（除）、%（取模）
加性运算符：+（加）、-（减）
移位运算符：<<（左移）、>>（右移）、>>>（无符号右移）
...
条件运算符（三元运算符）：? :
赋值运算符：=, +=, -=，*=, /=, %=, &=, |=, ^=, <<=, >>=, >>>=
请注意，虽然这个列表提供了一般性的指导原则，但在具体编程实践中，使用括号来明确指定计算顺序是一种好的编程习惯，尤其是在复杂的表达式中，这可以提高代码的可读性和可维护性。例如，(a + b) * c 明确地先执行加法，然后再进行乘法，无论运算符的默认优先级如何。
```